...
export function updateAllTokensBalance(appState1, userAddress, _tokenAddresses, force = false) {
    return __awaiter(this, void 0, void 0, function* () {
            ...
            if (isUserAddress(appState, userAddress)) {
                ...
                if (nonUpdateToken.length > 0) {
                    let [balances] = yield Promise.all([
                        multiCall(BEP20Abi, nonUpdateToken.map((_tokenAddress, _) => ({
                ...
                if (bnbIndex != -1) {
                    if (isNonUpdateTokenBalance(appState, userAddress, bnb) || force) {
                        appState = yield updateTokenBalance(appState, userAddress, bnb);
                    }
                }
            }
        }
