        ...
        try {
            if (appState.smartWalletState.liquidityCampainState.isFetch == false || force == true) {
                ...
                let [lockTime, epsData, maxTotalDeposit, TVLDatas, stakerRewardsToClaims, depositedDatas, decimalsOtherLpToken] = yield Promise.all([
                    (0, helper_1.multiCall)(IVault_json_1.default, stakedTokenAddress.map((address, _) => ({
                        address: address,
                        name: "LOCK_TIME",
                        params: [],
                    ...
                    (0, helper_1.multiCall)(BEP20_json_1.default, otherTokenInLpAddress.map((address, _) => ({
                        address: address,
                        name: "decimals",
                        params: [],
                    })), appState.web3, appState.chainId)
                ...
                for (let i = 0; i < vaultConfigList.length; i++) {
                    let [priceData] = yield Promise.all([
                        (0, helper_1.multiCall)(BEP20_json_1.default, [otherTokenInLpAddress[i], underlyingAddress[i]].map((address, _) => ({
                            address: address,
                            name: "balanceOf",
                            params: [lpAddress[i]],
                        })), appState.web3, appState.chainId)
